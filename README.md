...
bash & cluster computing on the vieques workshop server, Git/GitHub, Python & programming practice, AI-assisted workflows, and
statistical rigor.

All three workshops thread **one continuous project**: students build a `zfp36l2-analysis` repo — bash on vieques
...
themed sample. Server-specific values (module system, conda) are tagged `[VERIFY on vieques]` pending a check on the box.
...
- `guides.html` — the four step-by-step setup guides (Connect to vieques, SSH→GitHub, first repo, GPG signed commits [optional]), with per-OS tabs.
...
## Workshops 1–2 — paste-graded server labs

- Both pages keep their on-site sandbox (practice terminal / git sandbox) as a **warmup**, then a leveled
  **"lab on vieques"** (Beginner→Expert): students run real commands on the server, paste the stdout back into
...
- W2 Beginner wires GitHub↔vieques over **SSH for every student** (once, reused all program) and pushes the
